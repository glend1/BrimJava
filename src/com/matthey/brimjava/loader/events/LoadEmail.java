...
public class LoadEmail {
	public static EventGroups loadAll(String evType, EventGroups events) {
		...
		sql.query("select J_Event.id, J_Event.attachmentfk, J_Event.EventTypesFK, J_EventEmail.subject,"
				+ " J_EventEmail.html, J_EventEmail.addressgroup, j_event.eventFk, j_event.typefk "
				+ "from J_Event "
				+ "join J_EventTypes on J_EventTypes.id = J_Event.EventTypesFK "
				+ "join J_Type on J_Type.id = J_Event.TypeFK "
				+ "join J_EventEmail on J_EventEmail.id = J_Event.eventFk "
				+ "where J_EventTypes.Name = '" + evType + "' and J_Type.Name = 'Email'");
		try {
			while(sql.getResult().next()) {
				events.add(sql.getResult().getInt("attachmentfk"), (GenericEvent) new EmailEvent(
						sql.getResult().getInt("id"), sql.getResult().getInt("attachmentfk"), 
						sql.getResult().getInt("EventTypesFK"), sql.getResult().getInt("EventFK"),
						sql.getResult().getInt("typefk"), sql.getResult().getString("subject"), 
						sql.getResult().getString("html"), sql.getResult().getInt("addressgroup"))
				);
			}
